...
here to retain clearness) it turns out, that the values are string formatted. 
...
abbreviation "tstm" is written out. To delete plural suffixes, all 
"s"-characters at the end of the string are deleted. 
...
them in respect to the entire data is analyzed.
```{r}
mean(!is.na(economics.data$PROPDMGEXP) & !economics.data$PROPDMGEXP %in% c("b", "k", "m"), na.rm=TRUE)
mean(!economics.data$CROPDMGEXP %in% c("b", "k", "m"), na.rm=TRUE)
```
